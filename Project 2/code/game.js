...
function Gold(pos) {
  ...
  this.wobb = Math.random() * Math.PI * 4;
}
...
Level.prototype.obstacleAt = function(pos, size) {
  ...
  for (var y = yStart; y < yEnd; y++) {
    for (var x = xStart; x < xEnd; x++) {
      ...
      if (fieldType && fieldType != "star" && fieldType != "moon" ) return fieldType;
    }
  }
};
...
var wobbSpeed = 4, wobbDist = 0.08

Gold.prototype.act = function(step) {
  this.wobb += step * wobbSpeed;
  var wobbPos = Math.sin(this.wobb) * wobbDist;
  this.pos = this.basePos.plus(new Vector(0, wobbPos));
};
